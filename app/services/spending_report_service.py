...
class AnalysisReportService:
    ...
    def generate_monthly_report(
        ...
        # 1. 월별 소비 요약 조회
        summary = (
            self.spending_analysis_service.get_monthly_summary_by_month(
                user_id=user_id,
                month=month,
            )
        )
        ...
        # 2. 과소비 카테고리 조회
        overspending_data = (
            self.spending_service.get_monthly_overspending_categories(
                user_id=user_id,
                month=month,
            )
        )
    ...
    def get_monthly_report(
        ...
        # 반드시 summary가 아니라 report를 검사해야 함
        if not report:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=(
                    "생성된 AI 소비 분석 리포트가 없습니다. "
                    "먼저 AI 소비 분석 리포트를 생성해주세요."
                ),
            )
    ...
    def build_report_context(
        ...
        return {
            ...
            "monthly_summary": {
                ...
                "spending_change_rate": self.decimal_to_float(
    ...
    def extract_top_card(
        ...
        return max(
            ...
            key=lambda item: Decimal(
                str(item.get("card_amount", 0) or 0)
            ),
        )
    ...
    def decimal_to_float(
        ...
        """Decimal 또는 None 값을 float로 변환한다."""
